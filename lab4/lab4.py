    ...
    class Parser:
        ...
        def __find_tags(self):
            ...
            while i < len(self.data):
                if self.data[i] == "<":
                    ...
                    current_string += self.data[i]
                    if current_string[0:4] == "<!--" and current_string[-3:]:
                        self.data = self.data[:i+1] + current_string[0]+"/"+current_string[1:]+self.data[i:]
        ...
        def __unite_tags(self):
            for i in range(len(self.tags)):
                if self.tags[i].skip:
                    continue
                if self.tags[i].name[0:4] == "<!--" and self.tags[i].name[-3:]:
                    continue
                j = i
                flag = True
                while flag:
                    ...
                    if self.tags[i] == self.tags[j]:
                        if j == i + 1 + self.calc_skip_tags(i, j) and self.check_change_unit(i, j):
                            ...
                            if not self.tags[i].is_united_without_tags:
                                ...
                                self.tags[i].data = "\n" + "  " * (self.tags[i].depth + 1) + "- " + self.tags[i].data
                                self.tags[i].data += "\n" + "  " * (self.tags[i].depth + 1)
                            ...
                            while k < len(self.tags) and self.tags[k].depth > self.tags[j].depth:
                                ...
                                self.tags[i].add[-1][-1].skip = self.tags[k].skip
                                k += 1
        ...
        def calc_output(self):
            ...
            while i < len(self.tags):
                if self.tags[i].skip:
                    ...
                    continue
                if self.tags[i].name[0:4] == "<!--" and self.tags[i].name[-3:]:
                        self.output += "#" + self.tags[i].name[4:-3]+"\n" + "  " * self.tags[i + 1].depth
                elif self.tags[i].is_united_without_tags:
                    ...
                    while j < len(self.tags) and self.tags[j].depth > self.tags[i].depth:
                        if self.tags[j].skip:
                            j += 1
                            continue
                        if self.tags[j].name[0:4] == "<!--" and self.tags[j].name[-3:]:
                            self.output += "#" + self.tags[j].name[4:-3] + "\n" + "  " * (self.tags[j + 1].depth + 1)
                        elif j == len(self.tags) - 1:
                    ...
                    for k in range(len(self.tags[i].add)):
                        ...
                        for z in range(len(self.tags[i].add[k])):
                            if self.tags[i].add[k][z].name[0:4] == "<!--" and self.tags[i].add[k][z].name[-3:]:
                                self.output += "#" + self.tags[i].add[k][z].name[4:-3] + "\n" + "  " * (
                                            self.tags[i].add[k][z+1].depth + 1)
                            elif z == len(self.tags[i].add[k]) - 1:
